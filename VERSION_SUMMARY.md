...
### v2.4.1 - 系统配置优化版本
**发布日期**: 2025年8月5日  
**主要功能**: 调度时间优化 + 文件输出路径修复 + 系统配置优化

#### 🆕 新增功能
1. **调度时间优化**
   - 将ingestion_scoring_pipeline从每天3次运行优化为每天1次运行
   - 调度时间调整为北京时间22:00，减少66%系统资源消耗
   - 简化运维管理，适合每日汇总分析

2. **文件输出路径修复**
   - 修复summary_generation_dag输出路径问题
   - 将文件输出从Jiuwanli/exports改为Jiuwanli/dev/exports
   - 优化Docker卷挂载配置，保持项目文件在项目目录内
...
#### 🔧 技术升级
- **调度优化**: 减少API调用频率，延长配额使用周期
- **路径修复**: 确保文件输出到正确位置，便于管理
- **配置优化**: 改进Docker配置，提升系统稳定性
- **文档完善**: 更新系统优化记录
...
**维护团队**: AInews 开发团队  
**最后更新**: 2025年8月5日 (v2.4.1) 

## 🗑️ 已下线功能与DAG（总览）

| 名称 | 版本变更 | 原因 | 替代方案 |
|---|---|---|---|
| fetch_twitter | v2.4.x 起下线 | 平台限制、信噪比低 | 暂无；必要时外部导入 |
| fetch_news | v2.4.x 起下线 | 旧抓取与评分流程合并 | `ingestion_scoring_pipeline` |
| make_daily_cards | v2.4.x 起下线 | 与新摘要产出重复 | `summary_generation_dag`（JSON输出） |
| parse_summary_logs | v2.4.x 起下线 | 与数据库直读重复 | `aggregate_daily_logs` 仅作调试/审计 | 